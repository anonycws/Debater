...
def poe_reply(text, bot, chatId=None):
    for chunk in poe_client.send_message(bot, text, chatId=chatId):
        pass
    return chunk
...
if __name__ == '__main__':
    pos_name, neg_name = "llama-2-chat-7b", "llama-2-chat-70b"
    ...
    for domain in data_name_list:
        ...
        if domain in ["education", "social", "fun filled", "family"]:
          continue

        for index in range(len(topics)): 
            if domain == "leisure" and index in [0, 1, 2, 3, 4, 5, 6]:
              continue
            pos_db_list = []
            neg_db_list = []
            ...
            print("##### llama2-chat-7b")
            pos_deb1_chunk = poe_reply(pos_role + pt_state, "Llama-2-7b")
            pos_chatId = pos_deb1_chunk["chatId"]
            pos_deb1 = pos_deb1_chunk["text"]
            print(pos_deb1)

            print("##### llama2-chat-70b")
            neg_deb1_chunk = poe_reply(neg_role + pt_state, "Llama-2-70b")
            ...
            pos_deb2 = poe_reply(m2_pos, "Llama-2-7b", chatId=pos_chatId)["text"]
            print(pos_deb2)

            print("##### llama2-chat-70b")
            m2_neg = neg_pt_f1.replace('TTTTT', pos_deb1).replace('LLLLL', pos_deb2)  # neg, first-free-debate; with neg-statement; with pos-db1
            neg_deb2 = poe_reply(m2_neg, "Llama-2-70b", chatId=neg_chatId)["text"]
            print(neg_deb2)
            ...
            pos_deb3 = poe_reply(m3_pos, "Llama-2-7b", chatId=pos_chatId)["text"]
            print(pos_deb3)

            print("##### llama2-chat-70b")
            m3_neg = pt_f2.replace('TTTTT', pos_deb3)
            neg_deb3 = poe_reply(m3_neg, "Llama-2-70b", chatId=neg_chatId)["text"]
            print(neg_deb3)
            ...
            pos_deb4 = poe_reply(m4_pos, "Llama-2-7b", chatId=pos_chatId)["text"]
            print(pos_deb4)

            print("##### llama2-chat-70b")
            m4_neg = pt_f3.replace('TTTTT', pos_deb4)
            neg_deb4 = poe_reply(m4_neg, "Llama-2-70b", chatId=neg_chatId)["text"]
            print(neg_deb4)
            ...
            pos_deb5 = poe_reply(m5_pos, "Llama-2-7b", chatId=pos_chatId)["text"]
            print(pos_deb5)

            print("##### llama2-chat-70b")
            m5_neg = pt_summ.replace("TTT", pos_deb5)
            neg_deb5 = poe_reply(m5_neg, "Llama-2-70b", chatId=neg_chatId)["text"]
            print(neg_deb5)
            ...
            with open('results-7b-70b/'+pos_name + "_" + neg_name + "_" + domain + "_" + file_names[index] + "_db2.json", "w") as wfile:
                json.dump(db_dic, wfile, indent=4)
            
            poe_client.delete_chat("Llama-2-7b", chatId=pos_chatId)
            poe_client.delete_chat("Llama-2-70b", chatId=neg_chatId)